-- Atomic onboarding profile persistence.
-- Keeps profile, role profile, qualification data, WASSCE results and LinkedIn
-- in one authenticated transaction so the UI cannot report a false partial save.
...
create or replace function public.save_profile_bundle(
  ...
  p_target_career text default null,
  p_interests text[] default '{}',
  p_pathways text[] default '{}',
  p_qualification_code text default null,
  p_qualification_name text default null,
  p_grading_scale text default null,
  p_overall_score text default null,
  p_qualification_metadata jsonb default '{}',
  p_wassce_results jsonb default '[]',
  p_qualification_results jsonb default '[]',
  p_account_role text default 'student',
...
declare
  ...
  v_qid uuid;
  r jsonb;
  v_role text := case when p_account_role in ('student','employer','employee') then p_account_role else 'student' end;
  v_linkedin text := nullif(trim(p_linkedin_url),'');
begin
  if v_user is null then raise exception 'Authentication required'; end if;
  if v_linkedin is not null and v_linkedin !~ '^https://(www\\.)?linkedin\\.com/in/[A-Za-z0-9._-]+/?$' then
  ...
  if not found then
    insert into public.profiles(
      ...
      account_role,whatsapp_number,linkedin_url,onboarded
    ) values (
      ...
      v_role,nullif(trim(p_whatsapp_number),''),v_linkedin,true
    );
  end if;
  ...
  if p_qualification_code is not null and trim(p_qualification_code)<>'' then
    ...
    values(v_user,coalesce(nullif(trim(p_country_code),''),'GH'),trim(p_qualification_code),
      coalesce(p_qualification_name,trim(p_qualification_code)),p_grading_scale,p_overall_score,coalesce(p_qualification_metadata,'{}'))
...
revoke all on function public.save_profile_bundle(text,text,text,text,text,text,text[],text[],text,text,text,text,jsonb,jsonb,jsonb,text,text,text) from public;
grant execute on function public.save_profile_bundle(text,text,text,text,text,text,text[],text[],text,text,text,text,jsonb,jsonb,jsonb,text,text,text) to authenticated;
