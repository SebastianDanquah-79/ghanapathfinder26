    ...
    <nav aria-label="Primary" className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background/95 backdrop-blur-xl pb-[env(safe-area-inset-bottom)] md:hidden">
      <ul className="grid grid-cols-6">
        {tabs.map(({ to, label, icon: Icon }) => {
          ...
          return (
            <li key={to}>
              <a href={to} aria-label={label} aria-current={active ? "page" : undefined} className={"flex min-h-[58px] flex-col items-center justify-center gap-0.5 text-[10px] font-medium " + (active ? "text-primary" : "text-muted-foreground")}>
                <span className={"grid h-7 w-11 place-items-center rounded-full " + (active ? "bg-primary/15" : "")}><Icon className="h-[18px] w-[18px]" /></span>
                {label}
              </a>
            </li>
          );
