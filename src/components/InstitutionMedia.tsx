...
const cacheKey = (url: string) => `ghanapathfinder:institution-media:v4:${url}`;
...
export default function InstitutionMedia({ websiteUrl, name, variant = "card" }: { websiteUrl?: string | null; name: string; variant?: "card" | "hero" }) {
  ...
  const fallback = (
    <div className="flex h-full w-full items-center justify-center bg-secondary px-6 text-center">
      <div className="max-w-xl">
        {logo ? <img src={logo} alt={`${name} logo`} className="mx-auto mb-3 h-16 w-16 object-contain" referrerPolicy="no-referrer" /> : <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-xl border border-border bg-background text-xl font-bold text-primary">{name.slice(0, 2).toUpperCase()}</div>}
        <p className="font-semibold text-foreground">{name}</p>
        <p className="mt-1 text-xs text-muted-foreground">Campus image will use the official site first, then public institutional media.</p>
      </div>
    </div>
  );
    ...
    return (
      <div className="relative overflow-hidden rounded-2xl border border-border bg-secondary">
        <div className="aspect-[16/6] min-h-48">
          {image ? <img src={image} alt={`${name} campus`} className="h-full w-full object-cover" loading="eager" referrerPolicy="no-referrer" onError={(e) => { e.currentTarget.style.display = "none"; }} /> : fallback}
        </div>
        <div className="absolute inset-x-4 bottom-4 flex items-end gap-3">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-xl border border-border bg-background/95 p-2 shadow-sm">
            {logo ? <img src={logo} alt={`${name} logo`} className="max-h-full max-w-full object-contain" referrerPolicy="no-referrer" /> : <span className="text-lg font-bold text-muted-foreground">{name.slice(0, 2).toUpperCase()}</span>}
          </div>
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-border bg-secondary">
      <div className="aspect-[16/8]">{image ? <img src={image} alt={`${name} campus`} className="h-full w-full object-cover" loading="lazy" referrerPolicy="no-referrer" /> : fallback}</div>
      <div className="absolute left-3 bottom-3 flex h-12 w-12 items-center justify-center rounded-lg border border-border bg-background/95 p-1.5 shadow-sm">{logo ? <img src={logo} alt={`${name} logo`} className="max-h-full max-w-full object-contain" loading="lazy" referrerPolicy="no-referrer" /> : <span className="text-sm font-bold text-muted-foreground">{name.slice(0, 2).toUpperCase()}</span>}</div>
