...
const UsageCounter = ({ className = "", inverse = false }: { className?: string; inverse?: boolean }) => {
  ...
  return (
    <p
      ...
      className={`inline-flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-xs sm:text-sm ${inverse ? "text-white/80" : "text-muted-foreground"} ${className}`}
    >
      <span className="inline-flex items-center gap-1.5">
        ...
        <span className={`font-semibold ${inverse ? "text-white" : "text-foreground"}`}>
          {data.live_now.toLocaleString("en-GB")}
        </span>
        live now
      </span>
      <span aria-hidden className={inverse ? "text-white/30" : "text-border"}>|</span>
      <span className="inline-flex items-center gap-1.5">
