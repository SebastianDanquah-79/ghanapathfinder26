        ...
        {emailSent ? (
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              We sent a confirmation link to <span className="text-foreground">{email}</span>. Click it to activate your GhanaPathFinder account.
            </p>
          </div>
        ) : (
          <>
            ...
            <label className="flex items-start gap-3 mb-4 p-3 rounded-xl border border-border bg-secondary/50 cursor-pointer">
              <input type="checkbox" checked={acceptedTerms} onChange={(e) => setAcceptedTerms(e.target.checked)} className="mt-0.5 h-5 w-5 shrink-0 accent-[hsl(var(--primary))]" aria-describedby="terms-help" />
            ...
            <button onClick={handleGoogle} disabled={loading || !acceptedTerms} className="w-full mb-5 px-4 py-3 rounded-lg border border-border bg-secondary text-foreground text-sm font-medium hover:bg-secondary/70 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input type="email" required placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)} maxLength={255} className="w-full px-4 py-3 rounded-lg bg-secondary border border-border text-foreground text-sm placeholder:text-muted-foreground focus:outline-hidden focus:ring-2 focus:ring-primary/50" />
              <input type="password" required minLength={6} placeholder="Password (min 6 characters)" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 rounded-lg bg-secondary border border-border text-foreground text-sm placeholder:text-muted-foreground focus:outline-hidden focus:ring-2 focus:ring-primary/50" />
            ...
            {mode === "signin" && (
              <p className="text-center text-sm mt-3">
                <button onClick={handleForgotPassword} className="inline-flex items-center justify-center min-h-[44px] px-3 text-muted-foreground hover:text-primary">Forgot password?</button>
              </p>
            )}
            <p className="text-center text-sm text-muted-foreground mt-3">
              {mode === "signin" ? "New to GhanaPathFinder?" : "Already have an account?"}{" "}
              <button onClick={() => setMode(mode === "signin" ? "signup" : "signin")} className="inline-flex items-center justify-center min-h-[44px] px-2 text-primary font-medium">
