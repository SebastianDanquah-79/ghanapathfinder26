...
export function Notifications(){
  ...
  const mark=async()=>{if(!user)return;const {error}=await sb.from("notifications").update({is_read:true}).eq("user_id",user.id);if(!error)await refetch();};
...
function RoleDashboard({role,title,description}:{role:"employee"|"employer"|"startup_founder";title:string;description:string}){
  ...
  const {data:opps=[]}=useQuery({queryKey:["role-opportunities"],queryFn:async()=>{const {data,error}=await sb.from("opportunities").select("*").eq("is_active",true).order("created_at",{ascending:false}).limit(12);if(error)throw error;return (data??[]) as Row[];}});
  const post=async()=>{if(role!=="employer"||!user||!form.title.trim())return;await sb.from("opportunities").insert({title:form.title,company_name:form.company||String(profile?.full_name??"Employer"),application_url:form.url||null,type:form.type,is_active:true,posted_by:user.id,country:"Ghana",source:"manual"});setForm({title:"",company:"",url:"",type:"job"});};
