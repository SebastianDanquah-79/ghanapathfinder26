...
function RoleDashboard({role,title,description}:{role:"employee"|"employer"|"startup_founder";title:string;description:string}){
  ...
  const post=async()=>{if(role!=="employer"||!user||!form.title.trim())return;const { error } = await sb.from("opportunities").insert({title:form.title,company_name:form.company||String(profile?.full_name??"Employer"),application_url:form.url||null,apply_url:form.url||null,type:form.type,opportunity_type:form.type,is_active:true,posted_by:user.id,country:"Ghana",source:"manual"});if(error){alert(error.message);return;}setForm({title:"",company:"",url:"",type:"job"});};
