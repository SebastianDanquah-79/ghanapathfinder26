...
function parse(xml:string,category:string){return [...xml.matchAll(/<item>([\s\S]*?)<\/item>/g)].map(m=>{const b=m[1] ?? "";const tag=(n:string)=>{const x=b.match(new RegExp("<"+n+">([\\s\\S]*?)</"+n+">"));return x?.[1]?decode(x[1].replace(/<!\[CDATA\[|\]\]>/g,"").trim()):""};return {category,title:tag("title"),url:tag("link"),publishedAt:tag("pubDate"),source:tag("source")||"Google News"};}).filter(x=>x.title&&x.url);}
